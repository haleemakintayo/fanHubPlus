...
export default function FandomBot({ qaData, onShowToast }) {
  ...
  return (
    <>
      ...
      <div className="fixed bottom-6 right-6 z-40">
        {!isOpen && (
          <button
            ...
            className="group flex items-center gap-2.5 px-4 py-3 bg-[#FACC15] text-black font-black text-sm uppercase tracking-tight border-3 border-black brutal-shadow-lg brutal-btn relative"
          ...
          >
            ...
            <span className="absolute -top-1.5 -right-1.5 flex h-4 w-4">
              ...
              <span className="relative inline-flex rounded-full h-4 w-4 bg-[#F43F5E] border border-black text-[9px] text-white font-bold items-center justify-center">
                1
              </span>
            </span>

            <div className="p-1 bg-black text-[#FACC15] border border-black group-hover:rotate-6 transition-transform">
              <Bot className="w-5 h-5" />
            </div>
            <span>Ask FandomBot 🤖</span>
          </button>
        )}
      </div>
      ...
      {isOpen && (
        <div 
          className="fixed bottom-6 right-4 sm:right-6 z-50 w-[92vw] sm:w-[420px] bg-white dark:bg-[#161B22] border-3 border-black dark:border-white brutal-shadow-lg flex flex-col max-h-[80vh] h-[580px] animate-in slide-in-from-bottom duration-200"
          role="dialog"
          aria-modal="true"
          aria-labelledby="fandombot-header"
        >
          {/* Header */}
          <div className="bg-[#FACC15] text-black p-3.5 border-b-3 border-black flex items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              <div className="p-1 bg-black text-[#FACC15] border border-black">
                <Bot className="w-4 h-4" />
              </div>
              <div>
                <h3 id="fandombot-header" className="font-black text-sm uppercase tracking-tight leading-tight">
                  FANDOMBOT AI
                </h3>
                <span className="font-mono text-[10px] font-bold uppercase tracking-wider block">
                  ● ONLINE • SRS TECHWIZ 7
                </span>
              </div>
            </div>
