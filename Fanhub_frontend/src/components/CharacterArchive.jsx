      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-3 sm:gap-4 mb-6 sm:mb-8 pb-4 sm:pb-6 border-b-2 border-black dark:border-neutral-800">
          <div>
            ...
            <p className="text-neutral-700 dark:text-neutral-300 font-medium text-xs sm:text-sm md:text-base mt-1 max-w-2xl">
              Deep-dive character biographies, faction affiliations, power telemetry, and community-curated lore essays.
            </p>
          </div>

          <div className="flex flex-wrap items-center gap-2">
            <span className="font-mono text-[10px] sm:text-xs font-bold uppercase px-2.5 sm:px-3 py-1.5 bg-black text-[#FACC15] border-2 border-black">
              {displayedCharacters.length} {displayedCharacters.length === 1 ? 'Champion' : 'Champions'}
            </span>
            {onOpenSubmissionModal && (
              <button
                type="button"
                onClick={onOpenSubmissionModal}
                className="px-3 py-1.5 bg-[#34D399] text-black font-black text-[10px] sm:text-xs uppercase tracking-tight border-2 border-black brutal-shadow-sm brutal-btn flex items-center gap-1.5"
              >
                <PlusCircle className="w-3.5 h-3.5" />
                <span>Submit Character / Lore</span>
              </button>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 mb-6 sm:mb-8">
          <span className="inline-flex items-center gap-1 font-mono text-[10px] sm:text-xs font-black uppercase text-neutral-500 mr-1">
            <Filter className="w-3.5 h-3.5" /> Filter Class:
          </span>
          {CHARACTER_CATEGORY_FILTERS.map((cat) => {
